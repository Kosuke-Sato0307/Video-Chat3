...
```
  ...
  ブラウザ ──┼─ GET / , /style.css , /app.js              │
   (client) │      └→ Workers Static Assets (public/)     │
            ...
            │                                             │
            ┼─ GET /api/ice                               │
            │      └→ 公開 STUN ＋ Cloudflare Realtime TURN│
            │         の短命認証情報を発行                 │
            └─────────────────────────────────────────────┘
                            ▲  シグナリングのみ
...
2. `/api/ice` から ICE サーバー（公開 STUN ＋ TURN）を取得（失敗時は公開 STUN にフォールバック）。
3. `/api/room?room=合言葉` へ WebSocket 接続。
4. `welcome` 受信 → 既存メンバー全員へ **自分から** offer を送る（グレア回避ルール）。
5. ピアごとに `RTCPeerConnection`（公開 STUN ＋ TURN）を生成し、トラック送出・ICE 交換。
6. `track` イベントで相手の映像タイルを追加。`peer-leave` でタイル除去。
7. 接続が `failed` になったら、発信側が一度だけ ICE 再起動（`iceRestart`）を試みる。

### ICE サーバー（STUN ＋ TURN）

- ICE 設定は `/api/ice` が返す（`src/index.ts` の `buildIceServers`）。公開 STUN は常に含む。
- **TURN は Cloudflare Realtime TURN**（無料）。Worker が環境変数 `TURN_TOKEN_ID` /
  `TURN_API_TOKEN` を使って Cloudflare の API を呼び、**短命の TURN 認証情報**を発行する。
  API トークンはクライアントに渡さず Worker 側に隠蔽する。
- `TURN_TOKEN_ID` / `TURN_API_TOKEN` が未設定なら公開 STUN のみを返す（TURN なしでも動作）。
...
- TURN を設定すれば対称型 NAT・企業ネットワークでも接続できる。**TURN 未設定（公開 STUN のみ）**
  の場合は、対称型 NAT 等で P2P 接続が確立できないことがある。
